...
| configuration?.wpasupplicant_wait_time | number | <sup>*(optional)*</sup> Waiting time for wpa_supplicant to connect |
| configuration?.wpasupplicant_logfile | string | <sup>*(optional)*</sup> File name to save debug info from wpa_supplicant. Will be created in /tmp/WifiControl/ directory. Debug will be turned on if this is set |
