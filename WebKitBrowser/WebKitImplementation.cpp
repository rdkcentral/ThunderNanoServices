...
namespace Logging{
    // copied from Thunder R4_4 Source/messaging/Logging.cpp
    void DumpSystemFiles(const pid_t pid)
    {
        static auto logProcPath = [](const std::string& path) {
            ...
            if (fileStream.is_open()) {
                SYSLOG(Trace::Fatal, (_T("-== %s ==-\n"), path.c_str()));
                std::string line;
                
                while (std::getline(fileStream, line)) {
                    SYSLOG(Trace::Fatal, (line));
                }
            }
        };
        ...
        uint32_t Configure(PluginHost::IShell* service) override
        {
            ...
            if (!_config.LoggingTarget.Value().empty()) {
                if (!RedirectAllLogsToService(_config.LoggingTarget.Value())) {
                    SYSLOG(Trace::Error, (_T("Could not redirect logs to %s"), _config.LoggingTarget.Value().c_str()));
                }
            }
            #endif
            ...
            if (_config.DiskCacheDir.Value().empty() == false) {
                std::string path = _config.DiskCacheDir.Value();

                if (path.front() != '/') {
                    path = _service->PersistentPath() + path;
                    _config.DiskCacheDir = path;
                }
            ...
            if (_config.CookieStorage.Value().empty() == false) {
                std::string path = _config.CookieStorage.Value();

                if (path.front() != '/') {
                    path = _service->PersistentPath() + path;
                    _config.CookieStorage = path;
                }
            ...
            if (_config.LocalStorage.Value().empty() == false) {                
                std::string path = _config.LocalStorage.Value();

                if (path.front() != '/') {
                    path = _service->PersistentPath() + path;
                    _config.LocalStorage = path;
                }
            }
