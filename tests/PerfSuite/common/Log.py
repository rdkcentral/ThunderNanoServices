...
class Log:
    def __init__(self, name, verbose=False, warnings=True):
        self.warnings = []
        self.errors = []
        self.infos = []
    ...
    def __Print(self, text, file=""):
        ...
        print("%s: %s%s%s" % (current_time, file, ": " if file else "", text))
    ...
    def Warn(self, text, file=""):
        if self.show_warnings:
            ...
            self.warnings.append(
                "%s: %s%s: %s%s%s" % (self.name, self.cwarn, self.creset, file, ": " if file else "", text))
