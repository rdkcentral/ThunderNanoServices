...
namespace Tests {
    ...
    protected:
        ...
        static void SetUpTestSuite()
        {
            ...
            const uint32_t result = _runtime.Initialize(plugins, TEST_PLUGIN_PATH);
    ...
    TEST_F(TestPluginTest, COMRPC_EchoReturnsInput)
    {
        ...
        string output;
        const uint32_t result = iface->Echo("hello", output);
    ...
    TEST_F(TestPluginTest, COMRPC_GreetReturnsMessage)
    {
        ...
        string message;
        const uint32_t result = iface->Greet("Thunder", message);
        EXPECT_EQ(result, Core::ERROR_NONE);
    ...
    TEST_F(TestPluginTest, COMRPC_GreetDefaultsToWorld)
    {
        ...
        string message;
        const uint32_t result = iface->Greet("", message);
        EXPECT_EQ(result, Core::ERROR_NONE);
    ...
    TEST_F(TestPluginTest, COMRPC_EchoEmptyString)
    {
        ...
        string output;
        const uint32_t result = iface->Echo("", output);
        EXPECT_EQ(result, Core::ERROR_NONE);
    ...
    {
        string response;
        const uint32_t result = _runtime.Invoke("TestPlugin.echo", R"({"input":"hello"})", response);
    ...
    {
        string response;
        const uint32_t result = _runtime.Invoke("TestPlugin.greet", R"({"name":"Thunder"})", response);
    ...
    TEST_F(TestPluginTest, JSONRPC_SubscribeReceivesEvent)
    {
...

        {
            ...
            const bool received = cv.wait_for(lock, std::chrono::milliseconds(2000), [&] { return eventFired; });
