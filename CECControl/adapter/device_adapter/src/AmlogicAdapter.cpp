        ...
        private:
            ...
            public:
                    ...
                    , _descriptor(open(connector.c_str(), O_RDWR))
                {
                    ...
                    ioctl(_descriptor, CEC_IOC_SET_OPTION_ENALBE_CEC, 0x1);
                    ioctl(_descriptor, CEC_IOC_SET_OPTION_SYS_CTRL, 0x8);
                ...
                virtual ~Link()
                {
                    if (IsValid()) {
                        ...
                        ioctl(_descriptor, CEC_IOC_SET_OPTION_SYS_CTRL, 0x0);
                        ioctl(_descriptor, CEC_IOC_SET_OPTION_ENALBE_CEC, 0x0);
                ...
                {
                    Core::SafeSyncType<Core::CriticalSection> scopedLock(_lock);
                    return (ioctl(_descriptor, CEC_IOC_ADD_LOGICAL_ADDR, logical_address) >= 0) ? HDMI_CEC_ADAPTER_ERROR_OK : HDMI_CEC_ADAPTER_ERROR_SYSTEM;
                }

                cec_adapter_error_t ReleaseAddress(uint8_t logical_address) const
                {
                    Core::SafeSyncType<Core::CriticalSection> scopedLock(_lock);
                    return (ioctl(_descriptor, CEC_IOC_CLR_LOGICAL_ADDR, logical_address) >= 0) ? HDMI_CEC_ADAPTER_ERROR_OK : HDMI_CEC_ADAPTER_ERROR_SYSTEM;
                }

                cec_adapter_error_t PhysicalAddress(uint16_t& pa)
                {
                    Core::SafeSyncType<Core::CriticalSection> scopedLock(_lock);
                    return (ioctl(_descriptor, CEC_IOC_GET_PHYSICAL_ADDR, &pa) >= 0) ? HDMI_CEC_ADAPTER_ERROR_OK : HDMI_CEC_ADAPTER_ERROR_SYSTEM;
                }
