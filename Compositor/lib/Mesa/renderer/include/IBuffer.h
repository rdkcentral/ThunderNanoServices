...
namespace Interfaces {
    struct EXTERNAL IBuffer {
        ...
        virtual uint8_t Planes() const = 0; // The amount of graphic planes available in the buffer.

        virtual uint32_t Stride(const uint8_t PlaneId) const = 0; // Bytes per row for a plane [(bit-per-pixel/8) * width]
        virtual uint32_t Offset(const uint8_t PlaneId) const = 0; // Offset of the plane from where the pixel data starts in the buffer.
    }; // struct IBuffer
} // namespace Interfaces
} // namespace Compositor