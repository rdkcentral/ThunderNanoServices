...
namespace Plugin {
    ...
    class CompositorImplementation : public Exchange::IComposition, public Exchange::IComposition::INotification {
    private:
        ...
        public:
            ExternalAccess(const Core::NodeId & source, IComposition::INotification* parentInterface)
    ...
    public:
        ...
        uint32_t Configure(PluginHost::IShell* service) 
        {
            ...
            _externalAccess.reset(new ExternalAccess(Core::NodeId(config.Connector.Value().c_str()), this));

            uint32_t result = _externalAccess->Open(Core::infinite);

            if (result == Core::ERROR_NONE) {
                // Announce the port on which we are listening
                Core::SystemInfo::SetEnvironment(_T("RPI_COMPOSITOR"), config.Connector.Value(), true);
