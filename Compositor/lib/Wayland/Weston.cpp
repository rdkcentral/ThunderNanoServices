...
namespace Weston {
    ...
    private:
        ...
        public:
            ...
            {
                if (_parent->_surfaceSetTopAPI != nullptr) {
    ...
    public:
        ...
        ~Compositor() {
            ...
            _exitTimer = wl_event_loop_add_timer(loop, TerminateCallback, _compositor);
            wl_event_source_timer_update(_exitTimer, 1);
    ...
    private:
        ...
        static int TerminateCallback(void *data)
        {
            struct weston_compositor* compositor = static_cast<struct weston_compositor*>(data);
        ...
        static void NotifySurfaceActivate(struct wl_listener* listener, void* data)
        {
            ...
            if (surface->get_label) {
                ...
                if (status >= 0) {
                    ...
                    Compositor::UserData* userData = static_cast<Compositor::UserData*>
                            (weston_compositor_get_user_data(surface->compositor));
            ...
            if (library.IsLoaded() == true) {
                ...
                if (shellInit != nullptr) {
                    if (shellInit(_compositor, 0, NULL) == 0) {
                        _surfaceSetTopAPI = reinterpret_cast<ShellSurfaceSetTop>(
                                            library.LoadFunction(ShellSurfaceUpdateLayerAPI));
                        status = Core::ERROR_NONE;
                    }
                }
            }
